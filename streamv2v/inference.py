...
def load_image_as_video_tensor(
    ...
    if resize_hw is not None:
        img = img.resize(resize_hw[::-1], Image.BICUBIC)
    img_np = np.array(img)
    img_tensor = torch.from_numpy(img_np).float()  # [H, W, C]
    img_tensor = img_tensor.permute(2, 0, 1)  # [C, H, W]
    if normalize:
        img_tensor = img_tensor / 127.5 - 1.0
    # Repeat for num_frames
    video_tensor = img_tensor.unsqueeze(1).repeat(1, num_frames, 1, 1)  # [C, T, H, W]
    return video_tensor
